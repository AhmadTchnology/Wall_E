...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are Wall-E, Tony Stark's AI assistant. "
...
def _update_memory_async(user_text: str, wall_e_text: str) -> None:
    """
...
class WallELive:
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[Wall-E] 🔧 TOOL: {name}  ARGS: {args}")
    ...
    async def _listen_audio(self):
        ...
        try:
            ...
            print(f"[Wall-E] ❌ Mic error: {e}")
            raise
        finally:
            stream.close()

    async def _receive_audio(self):
        print("[Wall-E] 👂 Recv started")
        out_buf = []
        in_buf  = []
        ...
        try:
                ...
                async for response in turn:
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if in_buf:
                                full_in = " ".join([b.strip() for b in in_buf if b.strip()]).strip()
                                if full_in:
                    ...
                    if response.tool_call:
                        ...
                        for fc in response.tool_call.function_calls:
                            print(f"[Wall-E] 📞 Tool call: {fc.name}")
            ...
            print(f"[Wall-E] ❌ Recv error: {e}")
            traceback.print_exc()
            raise

    async def _play_audio(self):
        print("[Wall-E] 🔊 Play started")
        stream = await asyncio.to_thread(
            pya.open,
            format=FORMAT,
            channels=CHANNELS,
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    print("[Wall-E] ✅ Connected.")
                    self.ui.write_log("Wall-E online.")
                ...
                print(f"[Wall-E] ⚠️  Error: {e}")
                traceback.print_exc()

            print("[Wall-E] 🔄 Reconnecting in 3s...")
